...
@application.route('/geolocation', methods = ['GET'])
def geo_search():
    print(request.args["location"])
    geo_data = request.args["location"][1:-1].split(',')
    print(geo_data)
    # print(request.args["location"][0])
    # print(type(request.args["location"][0]))
    coords = []
    if request.method == 'GET':
        es_data = config.es.search(index=config.AWS_ES_INDEX, body=
        {
            "query": {
                "bool" : {
                    "must" : {
